...
static void check_pak_file_exists()
{
	...
	if (Sys_FileOpenRead(QUAKE_WII_BASEDIR"/id1/pak0.pak", &handle) < 0)
	{
		Sys_Error(
			...
			"\n"
			"Please make sure it is on your SD card/USB Drive in the correct\n"
			"location.\n"
			"\n"
			"If you are absolutely sure the file is correct, your SD\n"
			"card/USB Drive may not be compatible with the lib which\n"
...
void frontend(void)
{
	...
	else
	{
		do 
		{
			sys_netinit_error = if_config(sys_ipaddress_text, NULL, NULL, TRUE, 3);
